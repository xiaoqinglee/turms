...
public class TokenBucket {
    ...
    public boolean tryAcquire(long timestampNanos) {
        ...
        long refillInterval = context.refillIntervalNanos;
        if (refillInterval <= 0) {
            return false;
        }
        int periods = (int) ((timestampNanos - lastRefillTimeNanos) / refillInterval);
        if (periods <= 0) {
            return false;
        }
        // Try to refill.
    ...
    public void refill(long timeNanos) {
        ...
        int periods = (int) ((timeNanos - lastRefillTimeNanos) / refillIntervalNanos);
        if (periods <= 0) {
            return;
        }
        int tokenCount = tokens;
